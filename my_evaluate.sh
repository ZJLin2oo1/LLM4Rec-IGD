...
#SBATCH --job-name=my_evaluate
#SBATCH --gres=gpu:1                    # 分配 1 个 GPU
#SBATCH --cpus-per-task=16
#SBATCH --time=10:00:00                 # 作业运行时间
#SBATCH --output=log/my_evaluate.txt
...
#for category in "Sports_and_Outdoors" ; do
...
#    python ./code/split.py --input_path ${test_file} --output_path ./temp/${category}_base
    ...
    do
        echo $i
        CUDA_VISIBLE_DEVICES=$i python -u ./code/my_evaluate.py \
...
#        --reference_item_path ./items_pool/${category}_statistics.json\
#        --reference_item_path ${info_file} \